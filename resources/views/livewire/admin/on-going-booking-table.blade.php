                    ...
                    <tbody>
                        @forelse($on_going_bookings as $booking)
                            <tr class="text-center">
                                ...
                                <td>{{ ($booking->is_paid) ? $booking->payment->status : 'Not Applicable' }}</td>
                                ...
                                <td class="d-flex align-items-center justify-content-around gap-2">
                                    <button class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#viewBookingModal{{ $booking->id }}"><i class="fas fa-eye"></i></button>
                                    <button class="btn btn-sm btn-primary"><i class="fas fa-check"></i></button>
                                </td>

                                <!-- View Modal -->
                            <div class="modal fade" id="viewBookingModal{{ $booking->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-body">
                                       ...
                                       <div class="row">
                                            <div class="col-12 d-flex flex-column align-items-center">
                                                <img src="{{ asset('storage/'.$booking->reservation->room->image_path) }}" style="width: 700px; height:400px" alt="Category Image" class="img-fluid">
                                                <h4 class="modal-title fw-bold my-2 text-center" id="exampleModalLabel">{{ $booking->reservation->room->name }}</h4>
                                            </div>
                                        ...
                                        <div class="row my-2 px-5">
                                                ...
                                                <div class="col-6">
                                                    ...
                                                    <p>Customer Name: <strong>{{ $booking->reservation->user->name }}</strong></p>
                                                    <p>Email: <strong>{{ $booking->reservation->user->email }}</strong></p>
                                                    <p>Contact Number: <strong>{{ $booking->reservation->user->mobile_number }}</strong></p>
                                                    <p>Checked In/Out: <strong>{{ $booking->checked_in. " to ".$booking->checked_out }}</strong></p>
                                                    <p>Payment Status: <strong> 
                                                        @switch($booking->is_paid)
                                                            ...
                                                            <span class="badge badge-danger">Unpaid</span>
                                                                @break
                                                            @default
                                                            <span class="badge badge-primary">{{ $booking->is_paid }}</sp>
                                                        @endswitch
                                                            </strong> 
                                                    </p>
                                                    <p>
                                                        Payment Method : <strong>{{ ($booking->is_paid) ? $booking->payment->status : 'Not Applicable (Unpaid)' }}</strong>
                                                    </p>
                                                    <p>
                                                        Amount To Pay/Paid: <strong>₱{{ $booking->amount }}</strong>
                                                    </p>
                                                ...
                                                <div class="col-6 px-5">
                                                    ...
                                                    <p>Price per Night: <strong>₱{{ $booking->reservation->room->category->price }}</strong></p>
                                                    <p>Size (sqm): <strong>{{ $booking->reservation->room->category->size }}</strong></p>
                                                    <p>Capacity (max person): <strong>{{ $booking->reservation->room->category->capacity }}</strong></p>
                                                </div>
